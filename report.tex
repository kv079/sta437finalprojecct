...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig1_eda_overview.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig2_correlations.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig3_mvn_qqplot.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig4_pca.png}
...
\textbf{How many factors?} Panel~A of Figure~\ref{fig:fa} shows that log-likelihood
increases steeply from 1 to 2 factors but flattens sharply thereafter, supporting a
\textbf{2-factor model}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig5_factor_analysis.png}
...
tested by unsupervised GMM clustering. We fit a 3-component GMM to photometric
...
The result is stark: \textbf{ARI = 0.16} for photometry alone vs.\ \textbf{ARI = 0.91}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig6_gmm_clustering.png}
  ...
  (B) GMM with 3 components on photometric features: ARI = 0.16, reflecting poor recovery
  of the true class structure. (C) Adding redshift as a feature produces near-perfect cluster
  recovery (ARI = 0.91), demonstrating that redshift is the dominant discriminative variable.
  All panels show 3,000 random objects.}
  \label{fig:gmm}
\end{figure}

\textbf{What does ARI = 0.16 mean in practice?} The GMM can identify a quasar-like cluster
but cannot reliably separate stars from galaxies using photometry alone. This answers our
central question at the unsupervised level: photometry encodes \emph{some} class
information (primarily for quasars) but is insufficient for fully unsupervised classification.
...
\begin{itemize}
  \item \textbf{LDA, photo only:} $92.0 \pm 0.8\%$
  \item \textbf{LDA, photo+redshift:} $92.0 \pm 0.6\%$
  \item \textbf{LR, photo only:} $93.3 \pm 0.8\%$
  \item \textbf{LR, photo+redshift:} $97.1 \pm 0.2\%$
\end{itemize}

Two findings stand out. First, \textbf{LR substantially outperforms LDA on photo+redshift}
(97.1\% vs.\ 92.0\%), while both perform similarly on photo only. The reason is fundamental
to LDA's assumptions: LDA assumes equal class covariance matrices and implicitly assumes
near-MVN features. Redshift has a drastically different distribution in each class (near-zero
for stars, moderate for galaxies, large and skewed for quasars), violating the equal-covariance
assumption and limiting LDA's ability to exploit it. Logistic regression, which makes no
distributional assumption, exploits redshift's discriminative power freely.

Second, the gap between photo-only and photo+redshift is \textbf{much larger for LR
(3.8 percentage points) than for LDA ($\approx 0$ pp)}, confirming that LDA's assumptions
are the binding constraint when redshift is included.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig7_lda_cv.png}
  ...
  (photo+redshift) achieves 97.1\%, outperforming LDA by 5 percentage points because it
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig8_robust_kernel.png}
...
spectroscopic classes (GMM ARI = 0.16). Redshift is the dominant discriminative variable
(ARI = 0.91 with redshift). At the supervised level, photometric colours achieve 93.3\%
classification accuracy (logistic regression), rising to 97.1\% with redshift. The remaining
...
gradient) that achieve 93\% supervised classification accuracy but cannot unsupervisedly
recover the three object types. Spectroscopic redshift remains indispensable for both
unsupervised clustering and for pushing supervised accuracy above 97\%. These results
